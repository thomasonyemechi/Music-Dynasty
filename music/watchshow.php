<?php 
    ...
    //exit();
	if(isset($_GET['show']) && isset($_GET['shaShow'])){
		$show = $_GET['show']; $sha = $_GET['shaShow'];
		if(sha1($show) == $sha){
				...
				$.ajax({
                	...
                	if(res.length > 0) {
                		...
                		res.map((artist) => {
	                		...
	                		tagged.append(son)
	                		let color = (artist.useropt > 0) ? 'text-danger' : 'text-white' ;
	                		voteBtn = (vote > 0) ? `<i class="fa fa-heart ${color} vote" id=${artist.data.sn} data-artist=${artist.data.sn} 
											data-voted=${artist.useropt}  style="font-size:17px"> </i>` : ' ';
	                		sonContent = `
								<div class="">
									<ul class="main__list">
										<li class="single-item">
											...
											<div class="single-item__title">
												<h4><a href="artistshow.php?show=${show}&&artist=${artist.id}">${artist.data.name}</a></h4>
												<span><a href="javascript:;">${artist.data.stagename}</a></span>
											</div>
											<a href="javascript:;">${voteBtn}<big class="text-bold text-white id="big"  "> ${artist.votes}</big></a>
										</li>
									</ul>
								</div>
							`
                		...
                		document.getElementById('tagged').innerHTML = `
                			<div class="alert bg-danger">No Tagged Artist </div>
                		`
                	}
				})
			}
