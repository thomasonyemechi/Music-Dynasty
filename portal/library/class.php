...
class Profile
{
	...
	function __construct()
    {
        ...
        elseif (array_key_exists('FindProfile', $_POST)) {$this->FindProfile(); }
        elseif (array_key_exists('EditProfile', $_POST)){$this->EditProfile();}
        elseif (array_key_exists('EditProfile1', $_POST)) {$this->EditProfile1();}
        elseif (array_key_exists('EditProfile3', $_POST)) {$this->EditProfile3();}
        elseif (array_key_exists('EditProfile2', $_POST)) {$this->EditProfile2();}
        elseif (array_key_exists('ResendConfirmationLink', $_POST)) {$this->ResendConfirmationLink();}
        elseif (array_key_exists('InvestPack', $_POST)) {$this->InvestPack();}
        ...
        elseif (array_key_exists('FundWalletadmin', $_POST)) {$this->FundWalletadmin();}
        elseif (array_key_exists('RequestWithdrawal', $_POST)) {$this->RequestWithdrawal();}
        elseif (array_key_exists('ApproveWithdrawal', $_POST)) {$this->ApproveWithdrawal();}
        elseif (array_key_exists('findUpline', $_POST)) {$this->findUpline();}
        elseif (array_key_exists('LoginUsers', $_POST)) {$this->LoginUsers();}
        elseif (array_key_exists('ActivateAccount', $_POST)){$this->ActivateAccount();}
...
VALUES('$userid','$user','$a1','$a2','$a3','$a4','$a5','$a6','$a7','$a8','$a9','$a10','$b1','$b2','$b3','$b4','$ctime')") or die('Cannot Connect to Server');
        ...
        $this->emailer($email);
        //Promote Uplines
        if($this->stage1($a2)==10){ $this->stageUpdate($a2); }
     ...
     function findUpline($key){
        ...
        $sql = $db->query("SELECT * FROM user WHERE (sn='$key' OR a1='$key' OR a2='$key' OR a3='$key' OR a4='$key' OR a5='$key' OR a6='$key' OR a7='$key' OR a8='$key' OR a9='$key' OR a10='$key') AND active < 3 ORDER BY a1 ASC LIMIT 1") or die(mysqli_error());
    ...
    //Debits all types
        ...
        elseif($code==3){$r='Account Activation'; }
        elseif($code==4){$r=''; }
        elseif($code==5){$r=''; }

        //credits
        elseif($code==6){$r='Membership Fee'; }
        elseif($code==7){$r='Topup purchase'; }
        elseif($code==8){$r='Referral Bonus 1st Gen.'; }
        elseif($code==9){$r='Referral Bonus 2nd Gen.'; }
        elseif($code==10){$r='Referral Bonus 3rd Gen.'; }
        //Credit: Leadership Bonuses
        elseif($code==11){$r='Refferal Bonus.'; }
        elseif($code==12){$r='Spillover Bonus.'; }
        elseif($code==13){$r='Spillover Bonus.'; }
        elseif($code==14){$r='Spillover Bonus.'; }
        elseif($code==15){$r='Commit.'; }
        elseif($code==16){$r='Commit.'; }
        elseif($code==17){$r='Company.'; }
        elseif($code==18){$r='Spillover Bonus 8th Gen.'; }
        elseif($code==19){$r='Spillover Bonus 9th Gen.'; }
        elseif($code==20){$r='Spillover Bonus 10th Gen.'; }
        //Credit: Wallet Funding
        elseif($code==21){$r='Wallet Funding by Admin'; }        
        elseif($code==22){$r='Wallet funding by bank deposit'; }

        elseif($code==23){$r='Personal Sales Bonus'; }
        elseif($code==24){$r='Referral Sales Bonus'; }
        elseif($code==25){$r='Indirect Referral Sales Bonus'; }
        elseif($code==26){$r='1st 7 days smart offer'; }
        elseif($code==27){$r='Promotional Offers'; }
        elseif($code==28){$r='Top 10 Leaders Bonus'; }
        elseif($code==29){$r='Top 10 Sales people bonus'; }
        elseif($code==30){$r='Job Wages'; }
        elseif($code==31){$r='Investment Capital Retrieval'; }
        elseif($code==32){$r='Return on Investment'; }

        //Staff Credit
        elseif($code==41){$r='Basic Salary'; }
        elseif($code==42){$r='Supprise Bonus'; }
        elseif($code==43){$r='Transport Allowance'; }
        elseif($code==44){$r='Accommodation Subsidy'; }
        elseif($code==45){$r='Performance Bonus'; }
        elseif($code==46){$r='Health Support'; }
        elseif($code==47){$r='Expense Reinbursement'; }
        elseif($code==48){$r='Training Allowance'; }
        elseif($code==49){$r='Personal Project Support'; }
        elseif($code==50){$r=''; }
...
if($sql){
    ...
    $report="Invalid user";
    $count=1;
} }else{$count=1; $report='Wrong Password';}
return;
    }
    ...
    {
        if($status==9){$r='<font color="green">Transaction Complete</font>'; }
        elseif($status==1 OR $status==0){$r='<font color="red">Awaiting Confirmation</font>'; }
        elseif($status==2){$r='<font color="blue">Transaction Processing</font>'; }
        elseif($status==3){$r='<font color="#036">Ready for Delivery</font>'; }
        else{$r='Pending';}
    ...
    function ApproveFundOrder(){
        ...
        $count = 1; $report = 'Transaction cannot be processed';       }
        }
 ...
 $report = 'New Password Mismatch, Try Again';
    ...
    function VerifySponsor()
    {
      ...
      if(isset($_GET['ref'])){
       ...
       else{$report='Invalid Referral Link'; $count=1;
       $_SESSION['signup']=1;
        }
       ...
       if(sqL1('user','sn',$sponsorkey)==1){
        ...
        $_SESSION['signup']=2;
       $report = 'Sponsor successfully verified'; }
       else{$report='You have entered an invalid sponsor ID. Confirm and try again'; $count=1; }
      }
 ...
 function EditProfile3()
    {
        ...
        $sql = $db->query("UPDATE user SET firstname ='$firstname', lastname ='$lastname', phone ='$phone',
            sex ='$sex', state ='$state', country ='$country' WHERE id ='$id' ")or die(mysqli_error());
        if ($sql) {
            $report = "Updated Successfully";
        } else {
            $count = 1;
...
///////////////1222112
    ...
    function PayReferral($paylist2){
        ...
        if ($b>0) {
            $id = $this->keyToId($b);
 if ($e = 0) { $amount=$this->ref1; }elseif($e=1){ $amount=$this->ref2; }else{ $amount=$this->ref3; }
            $type = $e + 8;
    ...
    function signupUserIni()
    {
        ...
        $password2= $_POST['password2'];  $ctime = time();
...
VALUES('$id','$sponsorkey','$a1','$a2','$a3','$a4','$a5','$a6','$a7','$a8','$a9','$a10','$b1','$b2','$b3','$firstname','$lastname','$phone','$email','$gender','$state',',$country','$username','$pwd','$pin','$ctime')") or die('Cannot Connect to Server');
      ...
      if($reg){  $sql = $db->query("UPDATE pin SET status = 1, id = '$username' WHERE pin = '$pin' ") or die(mysqli_error());  }
      $nd = sqL1('user', 'a1', $upline);
      $upd = $db->query("UPDATE user SET active='$nd' WHERE sn = '$upline' ");
...
return $num;
    ...
    {
        global $db;
        $que = $db->query("SELECT * FROM user WHERE id = '$user' ") or die(mysqli_error());
        $ro = mysqli_fetch_array($que);
        $val = ($col == '')?$ro['firstname'] . ' ' . $ro['lastname']:$ro[$col];
        return $val;
    }
...
//if(isset($report)){   unset($_SESSION['report']);  }
    ...
    {
        global $db, $report, $count;
        $username = strtolower(sanitize($_POST['user']));
        $password = $_POST['pass'];
        ...
        if (mysqli_num_rows($sql) > 0) {
            ...
            } else { $report = 'Invalid Login details pass, Try again'; $count = 1; }
        } else { $report = 'Invalid Login details emal, Try again'; $count = 1; }
        return;
    ...
    function ResetPassword()
    {
        ...
        if($password == $password2){
            ...
            $report = 'Password Mismatch, Please try again'; $count=1;
        }
    }
