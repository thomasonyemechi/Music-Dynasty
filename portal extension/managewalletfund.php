  ...
  <div class="content-wrapper">
    ...
    <!-- Main content -->
      ...
      <div class="container-fluid">
        <div class="row">
          <!-- left column -->
          <?php if(isset($_GET['transaction'])){  $trno = $_GET['transaction'];
          $sq = $db->query("SELECT * FROM walletorder WHERE trno = '$trno' ");
          $ro = mysqli_fetch_assoc($sq); $trid = $ro['id'];  if($ro['status']==0 AND isset($trid)){
                    ?>     
          <div class="col-md-12">
            <!-- jquery validation -->
          ...
          <?php }} ?>
          <?php if(isset($_GET['Reference'])){  $trno = $_GET['Reference'];
$sq = $db->query("SELECT * FROM wallet WHERE trno = '$trno' ");
$ro = mysqli_fetch_assoc($sq); $trid = $ro['id'];  
if(isset($trid)){
                    ?>
            <!-- /.card -->
            </div>
